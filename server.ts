...
// Global safe generation wrapper that intercepts quota exceptions and triggers the circuit breaker
async function safeGenerateContent(params: {
  ...
  try {
    ...
    if (isRateLimit) {
      triggerGeminiCooldown(12); // Activate circuit breaker for 12 minutes
    }
    throw error;
  }
}
...
function getFallbackChatResponse(message: string, category: string): string {
  ...
  if (category === "btech" || msg.includes("engineer") || msg.includes("computer") || msg.includes("algorithm") || msg.includes("circuit") || msg.includes("math")) {
...
3. **Decouple Dependencies (25min):** Verify typescript structures.`;
  ...
  if (category === "software" || msg.includes("coding") || msg.includes("code") || msg.includes("api") || msg.includes("bug") || msg.includes("developer")) {
...
3. **Audit for Render Quirks (25min):** Lint the workspace to remove empty hooks and typing bugs.`;
  ...
  if (category === "farms" || msg.includes("agriculture") || msg.includes("crop") || msg.includes("soil") || msg.includes("farm") || msg.includes("water") || msg.includes("irrigation") || msg.includes("agri")) {
...
3. **Analyze Case Studies (25min):** Analyze automated agricultural setups deployed in semi-arid zones.`;
  ...
  if (category === "english" || msg.includes("grammar") || msg.includes("english") || msg.includes("speak") || msg.includes("conversation") || msg.includes("vocabulary")) {
...
Speak short 1-minute summaries of your study note cards out loud every day. Pay close attention to word emphasis and spacing.`;
  ...
  if (category === "ainotes" || msg.includes("note") || msg.includes("write") || msg.includes("syllabus") || msg.includes("flashcard") || msg.includes("summary")) {
...
Keep the markdown clean, structured, and complete. Avoid saying "Sure, here's..." or "Hope this helps." Start immediately with '# 1. Goal Analysis'.`;
    ...
    console.error(`[Goal Coach Failover] Active fallback for goal: ${goal}`, error);
